...
const handleBlur = () => {
  setTimeout(() => {
    // Only shift focus if dropdown is still open (prevents auto close behavior)
    if (showDropdown.value && selectBar.value) {
      selectBar.value.focus?.(); // optional chaining for safety
    }
  }, 150);
};
